...
let autoexit = () => Object.keys(require.cache).forEach(key => require('fs').watch(key, () => process.exit(0)));
...
let npmRun = () => require('child_process').exec(require('./package.json').scripts[process.argv[2]]).stdout.pipe(process.stdout);
...
var entropy = (s) => [...new Set(s)].map(c => s.split('').filter(d => d == c).length).reduce((e, freq) => e + freq * Math.log2(s.length / freq), 0);
